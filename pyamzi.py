...
if __name__ == '__main__':
    eng = Engine("test")
    #eng.load(r"code\duckworld\duckworld")
    eng.load(r"code\rubik\rubik")
    print(eng.call_str("init_solve(random)"))
    print(eng.call_str("random_cube(X),pieces(X, P)"))
    #eng.main()
    #eng.call_str("banner")
    #print(eng.call_str("do(manual)"))

    if False:
        #term = eng.make_term("test(1, 2, test2(a, b))")
        #print(term, term._get_arg_types())
        eng.consult("test")

        eng.call_str("write(hello(world))")

        term = eng.call_str("test(aaa, bbb)")
        print(term)

        for res in eng.find_all("test(X, Y)"):
            print(res)

        for res in eng.find_all("sibling(mary, X)"):
            print(res)

        term = eng.make_term("test(あいう, abc)")
        print(term.to_object())

        term = eng.make_term("[1, [2, 3], 4]")
        print(term.to_object())

        term = eng.make_term("[]")
        print(term.to_object())

        term = eng.make_term("[[a]]")
        print(term.type_id)
        print(term.head.type_id)
        print(term.to_object())

        #print(term.functor, term.type_id)
        term = eng.make_term("123")
        assert term.to_object() == 123
        term = eng.make_term("123.5")
        assert term.to_object() == 123.5
        term = eng.make_term("abc")
        assert term.to_object() == "abc"
